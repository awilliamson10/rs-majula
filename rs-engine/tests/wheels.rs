...
/// process-global state (`wheels.rs`'s ★ comment: process-global is
/// deliberate). Rust's default test harness runs the tests in this file
/// concurrently across threads, so without this lock
